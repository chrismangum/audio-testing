...
app.controller 'main', ['$scope', '$routeParams', ($scope, $routeParams) ->
  ...
  checkArtist = (track) ->
    unless artist = _.findWhere $scope.artists, {name: track.artist}
      createArtist track
    else
  ...
  checkGenre = (track) ->
    unless genre = _.findWhere $scope.genres, {name: track.genre}
      createGenre track
    else
  ...
  socket.on 'json', (data) ->
    ...
    $scope.artists = _.compact _.map _.groupBy($scope.songs, 'artist'), (songs, artistName) ->
    ...
    $scope.genres = _.compact _.map _.groupBy($scope.songs, 'genre'), (songs, genreName) ->
